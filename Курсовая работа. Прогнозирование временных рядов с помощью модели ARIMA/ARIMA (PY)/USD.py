...
# Загружаю данные из .txt, который хранится по пути: "C:/Users/danik/Desktop/Курсач/USD_Corrected_Month.txt"
data = pd.read_csv('C:/Users/danik/Desktop/Курсач/USD_Corrected_Month.txt', delimiter='\s+', header=None, names=['Date', 'Value'], converters={'Value': lambda x: float(x.replace(',', '.'))})
...
pred_dynamic = results.get_prediction(start=pd.to_datetime('2022-03-01'), dynamic=True, full_results=True)
pred_dynamic_ci = pred_dynamic.conf_int()


# Рисую график прогноза и действительного значения
ax = y['2021':].plot(label='Known values', figsize=(20, 15))
...
ax.fill_betweenx(ax.get_ylim(), pd.to_datetime('2022-03-01'), y.index[-1],
alpha=.1, zorder=-1)
ax.set_xlabel('Date')
ax.set_ylabel('USD rate') # CO2 rate
plt.legend()
plt.show()
...
y_truth = y['2022-03-01':]              # Вычислить среднеквадратичную ошибку
