...
# Загружаю данные из .txt, который хранится по пути: "C:/Users/danik/Desktop/Курсач/Passengers_On_The_Airline_Data.txt"
data = pd.read_csv('C:/Users/danik/Desktop/Курсач/Bus.txt', delimiter='\s+', header=None, names=['Date', 'Value'], converters={'Value': lambda x: float(x.replace(',', '.'))})
y = pd.Series(data['Value'].values, index=pd.to_datetime(data['Date']))
print("\nДанные:\n", y, sep="")
...
pred_dynamic = results.get_prediction(start=pd.to_datetime('1959-03-01'), dynamic=True, full_results=True)
pred_dynamic_ci = pred_dynamic.conf_int()


# Рисую график прогноза и действительного значения
ax = y['1957':].plot(label='Known values', figsize=(20, 15))
...
ax.fill_betweenx(ax.get_ylim(), pd.to_datetime('1959-03-01'), y.index[-1],
alpha=.1, zorder=-1)
ax.set_xlabel('Date')
ax.set_ylabel('Passenger rate') # CO2 rate
plt.legend()
plt.show()
...
y_truth = y['1959-03-01':]              # Вычислить среднеквадратичную ошибку
mse = ((y_forecasted - y_truth) ** 2).mean()
print('Среднеквадратическая ошибка прогнозов равна: {}'.format(round(mse, 2)))


# Создание и визуализация прогноза. Получить прогноз на 500 шагов вперёд
